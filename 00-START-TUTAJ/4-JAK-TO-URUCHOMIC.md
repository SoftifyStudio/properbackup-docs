...
- **OVH Kimsufi KS-STOR**, IP **`51.255.93.127`**, Gravelines (FR). Koszt 109 zł netto/mc (~135 brutto).
- Sprzęt: Xeon-D 1521, 16 GB ECC, **4×4 TB HDD RAID5 ≈ ~11 TB** + 500 GB NVMe.
- **Proxmox VE** na NVMe → panel `https://51.255.93.127:8006` (root).
...
## Dostęp
- SSH do hosta Proxmox: `ssh root@51.255.93.127` — hasło w secret **`OVH_DEDICATED_SERVER_PROXMOX_ROOT_PASSWORD`** (globalny, w każdej sesji Devina).
